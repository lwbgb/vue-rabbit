...
const spliter = '★';
// 根据skus数据得到路径字典对象
const getPathMap = (skus: Array<GoodDetail>) => {
  ...
  console.log(`getPathMap pathMap:`, pathMap);

  return pathMap;
};

// 初始化禁用状态
function initDisabledStatus(specs: Array<any>, pathMap: Record<string, Array<string>>) {
...
// 更新按钮的禁用状态
const updateDisabledStatus = (specs: Array<any>, pathMap: Record<string, Array<string>>) => {
  // 遍历每一种规格
...
export default {
  ...
  emits: ['change'],
  setup(props, { emit }) {
    let pathMap: Record<string, Array<string>> = {};
    watchEffect(() => {
      // 得到所有字典集合
